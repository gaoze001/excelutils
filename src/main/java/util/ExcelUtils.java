...
//        return readExcelManySheet( file);
//    }
...
//    private static Map<String, JSONArray> readExcelManySheet( File file) throws IOException {
...
//    }
    ...
    private static JSONArray readExcel( File file) throws IOException {
        Workbook book = getWorkbook( file);
        if (book == null) {
        ...
        switch (cellType) {
            ...
            case Cell.CELL_TYPE_NUMERIC: // 数字
                ...
                if (HSSFDateUtil.isCellDateFormatted(cell)) {
                    ...
                    cellValue = bd.setScale(2, RoundingMode.HALF_EVEN ).toPlainString();// 数值 这种用BigDecimal包装再获取plainString，可以防止获取到科学计数值
                }
                break;
...
//    }
    ...
    public static <T> void exportTemplate( String fileName, Class<T> clazz) {
        exportTemplate( fileName, fileName, clazz, false);
    }

    public static <T> void exportTemplate( String fileName, String sheetName,
                                          Class<T> clazz) {
        exportTemplate( fileName, sheetName, clazz, false);
    }

    public static <T> void exportTemplate( String fileName, Class<T> clazz,
                                          boolean isContainExample) {
        exportTemplate( fileName, fileName, clazz, isContainExample);
    }
    ...
    public static <T> void exportTemplate( String fileName, String sheetName,
        ...
        // 导出数据
        export( fileName, sheetName, sheetDataList, selectMap);
    }
    ...
    public static void exportFile(File file, List<List<Object>> sheetData) {
        ...
        export( file, file.getName(), map, null);
    }
    ...
    public static void exportEmpty( String fileName) {
        List<List<Object>> sheetDataList = new ArrayList<>();
        List<Object> headList = new ArrayList<>();
        headList.add("导出无数据");
        sheetDataList.add(headList);
        export( fileName, sheetDataList);
    }

    public static void export( String fileName, List<List<Object>> sheetDataList) {
        export( fileName, fileName, sheetDataList, null);
    }

    public static void exportManySheet( String fileName, Map<String, List<List<Object>>> sheetMap) {
        export( null, fileName, sheetMap, null);
    }


    public static void export( String fileName, String sheetName,
                              List<List<Object>> sheetDataList) {
        export( fileName, sheetName, sheetDataList, null);
    }
    ...
    public static void export( String fileName, String sheetName,
        ...
        export( null, fileName, map, selectMap);
    }

    public static <T, K> void export( String fileName, List<T> list, Class<K> template) {
        // list 是否为空
        boolean lisIsEmpty = list == null || list.isEmpty();
        // 如果模板数据为空，且导入的数据为空，则导出空文件
        if (template == null && lisIsEmpty) {
            exportEmpty( fileName);
            return;
        }
        // 如果 list 数据，则导出模板数据
        if (lisIsEmpty) {
            exportTemplate( fileName, template);
            return;
        }
        // 导出数据
    ...
    public static void export( String fileName, List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap) {
        export( fileName, fileName, sheetDataList, selectMap);
    }

    private static void export( File file, String fileName,
